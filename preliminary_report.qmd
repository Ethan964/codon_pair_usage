...
'final-filtered-mut-netphos.csv' dataset. The target variable that we are 
...
analysis, we observed that position was highly predictive, bordering 
...
the model's ability to learn and pick up trends within the data. To account for
this, we conducted modeling both with and without position. This allows us to 
compare performance and retain flexibility in case position turns out to be 
essential for intepretation in later stages of the study.
...
We trained two types of tree-based ensemble models:
...
more useful to apply on data that can be harder to classify, but performs better
on structured data.
...
+ ROC AUC (Receiver Operating Characteristic): Measures the model's ability to
...
The initial modeling effort has highlighted both promising results and areas
where additional clarification would be valuable. To further improve model
interpretability and performance, we welcome insight on several aspects of the
dataset's structure, design choices, and biological context. 
